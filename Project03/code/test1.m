...
pde = data_1;                                    % choose initial data
                                                 % 1:smooth  2:continuous  3:piecewise continuous
option.taulist = [1/2 *option.hlist.^2, option.hlist.^1, 3*option.hlist.^2,option.hlist.^1, option.hlist.^1,option.hlist.^2];
option.mulist = option.taulist./option.hlist.^2; % mesh ratio
option.thetalist = [0 *ones(5,1), 1 *ones(5,1),1 *ones(5,1),1/2 *ones(5,1),0.5 - 1./(12*option.mulist(:,5)),0.5 - 1./(12*option.mulist(:,6))];
...
for ind = 1:3
    ...
    for k=1:6
        ...
        filename1 = ['Ne_1',num2str(ind),num2str(k),'.dat'];
        filename2 = ['Ce_1',num2str(ind),num2str(k),'.dat'];
        ...
        fclose(fid);
        disp(['finish ',num2str((ind-1)*6+k),'/',num2str(18)])
    end
end
%% Display data & figure
